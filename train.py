...
def train(epoch):
    ...
    for idx, (inputs, labels) in enumerate(train_loader):
        ...
        _, predict0 = torch.topk(outputs[0], 5)
        _, predict1 = torch.topk(outputs[1], 5)
        _, predict2 = torch.topk(outputs[2], 5)
        total += labels[0].size(0)
        # total += labels[1].size(0)
        # total += labels[2].size(0)
        correct0 += predict0.eq(labels[0].view(-1, 1).expand_as(predict0)).cpu().sum().double()
        correct1 += predict1.eq(labels[1].view(-1, 1).expand_as(predict1)).cpu().sum().double()
        correct2 += predict2.eq(labels[2].view(-1, 1).expand_as(predict2)).cpu().sum().double()
